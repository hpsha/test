...
class Products_Model extends CI_Model {
	...
	public function get_product()
			  {
		$where = "product_act=1";
		return $this->db->select('product_rand,product_id,product_product')->from('tbl_product')->where($where)->get()->result();
	}
	public function addproduct($data){
		$where = "product_product =" . "'" . $data['product_product'] . "'";
    ...
    public function edit_product($key){
		...
		return $this->db->select('product_rand,product_product')->from('tbl_product')->where($where)->get()->row();
	}
